...
function addTransactionDOM(transaction) {
  ...
  item.innerHTML = `
        ${transaction.text} <span>${formatCurrency(sign === '-' ? -Math.abs(transaction.amount) : Math.abs(transaction.amount))}</span>
<button class="delete-btn" onclick="removeTransaction(${transaction.id})"><i data-lucide="badge-x" class="lucide"></i></button>
    `;
...
document.getElementById('money-plus').innerHTML = formatCurrency(income);
document.getElementById('money-minus').innerHTML = formatCurrency(-expense);
